...
case 'film': {
  try {
    ...
    // ── Direct-link resolver: scrapes wrapper page for real MP4 ──
    const resolveDirectLink = async (wrapperUrl) => {
      try {
        const html = await axios.get(wrapperUrl, {
          timeout: 20000,
          headers: {
        ...
        const directMatch =
          html.match(/href="(https?:\/\/[^"]+\.(mp4|mkv|avi)[^"]*)"/i) ||
          html.match(/href="(https?:\/\/google\.com\/server[^"]*)"/i) ||
          html.match(/"(https?:\/\/[^"]*\.mp4[^"]*)"/i);

        const tgMatch = html.match(/href="(https?:\/\/t\.me\/[^"]+)"/i);

        return {
          direct:   directMatch ? directMatch[1] : null,
          telegram: tgMatch     ? tgMatch[1]     : null
        };
      } catch { return { direct: null, telegram: null }; }
    };
    ...
    const attachSmH = () => {
      const smH = async (u) => {
        ...
        } catch { socket.ev.off('messages.upsert', smH); }
      };
    ...
    // ── Quality picker (shared by movie & episode paths) ─────────
    const showQualityPicker = async (downloads, contextMsg, title) => {
      const dlLines  = downloads.map((d, i) => `*${i + 1}.* 🎞️ ${d.quality}`).join('\n');
      const cleanT   = String(title).split('|')[0].trim().slice(0, 50);
      const qCaption = `⬇️ *Quality Select කරන්න*\n╭━━━━━━━━━━━━━━━━━━━●\n┃ 🎬 *${cleanT}*\n╰━━━━━━━━━━━━━━━━━━━●\n\n${dlLines}\n\n━━━━━━━━━━━━━━━━━━━━\n*│ Reply with quality number*\n*│00 . ʙᴀᴄᴋ ᴛᴏ ᴍᴇɴᴜ*\n*│94 . ᴄʜᴇᴄᴋ ʙᴏᴛ ꜱᴘᴇᴇᴅ*\n> 🏷️ *KEZU TECH* | _TEAM DCT OFC_`;

      await socket.sendMessage(sender, { text: qCaption }, { quoted: contextMsg });
      attachSmH();

      return new Promise((resolve) => {
        const qH = async (u) => {
          try {
            const qMsg  = u.messages?.[0];
            if (!qMsg?.message || qMsg.key.remoteJid !== sender) return;
            const qText = (qMsg.message?.conversation || qMsg.message?.extendedTextMessage?.text || '').trim();
            if (qText === '00' || qText === '94') return;
            const qNum  = parseInt(qText, 10);
            if (isNaN(qNum) || qNum < 1 || qNum > downloads.length) return;
            socket.ev.off('messages.upsert', qH);
            resolve({ chosen: downloads[qNum - 1], qMsg });
          } catch { socket.ev.off('messages.upsert', qH); resolve(null); }
        };
        socket.ev.on('messages.upsert', qH);
        setTimeout(() => { socket.ev.off('messages.upsert', qH); resolve(null); }, 5 * 60 * 1000);
      });
    };

    // ── Resolve link → send as WhatsApp document, fallback to link ─
    const sendResolvedLink = async (wrapperUrl, quality, refMsg, title) => {
      await socket.sendMessage(sender, { react: { text: '⬇️', key: refMsg.key } });
      await socket.sendMessage(sender, {
        text: `⏳ *${quality}* download link resolve කරමින්...\n_Real link හොයමින් ඉන්නවා..._`
      }, { quoted: refMsg });

      const { direct, telegram } = await resolveDirectLink(wrapperUrl);
      const cleanTitle   = String(title).split('|')[0].trim().slice(0, 60);
      const safeFileName = cleanTitle.replace(/[^\w\s]/g, '').trim().replace(/\s+/g, '_') + `_${quality}.mp4`;

      // ── Try to send as WhatsApp document (streams file directly) ─
      if (direct) {
        try {
          await socket.sendMessage(sender, {
            text: `📥 *File upload කරමින්...*\n🎬 *${cleanTitle}*\n🎞️ *${quality}*\n\n_විශාල file නම් upload වෙන්න ටිකක් කාලයක් යයි... ☕_`
          }, { quoted: refMsg });
          ...
          await socket.sendMessage(sender, { react: { text: '✅', key: refMsg.key } });
          attachSmH();
          return;
        } catch (dlErr) {
          console.error('[CINE doc-send failed]', dlErr.message);
          // fall through to link fallback below
        }
      }

      // ── Fallback: send links as text ──────────────────────────────
      let caption = `📥 *Download Ready!*\n\n🎬 *${cleanTitle}*\n🎞️ *Quality:* ${quality}\n\n`;
      if (direct)        caption += `🔗 *Direct Download Link:*\n${direct}\n\n> ↑ IDM / UC Browser / ADM වලින් open කරන්න\n`;
      else if (telegram) caption += `📲 *Telegram:*\n${telegram}\n`;
      else               caption += `🔗 *Link:*\n${wrapperUrl}\n\n> ↑ Browser / IDM / UC Browser open කරන්න`;
      caption += `\n━━━━━━━━━━━━━━━━━━━━\n*│00 . ʙᴀᴄᴋ ᴛᴏ ᴍᴇɴᴜ*\n*│94 . ᴄʜᴇᴄᴋ ʙᴏᴛ ꜱᴘᴇᴇᴅ*\n> 🏷️ *KEZU TECH* | _TEAM DCT OFC_`;
      await socket.sendMessage(sender, { text: caption }, { quoted: refMsg });
      attachSmH();
    };
    ...
    const resultLines = results.map((r, i) => {
      const icon = isTV(r.link) ? '📺' : '🎬';
      const n    = String(i + 1).padStart(2, '0');
      return `*${n}.* ${icon} *${r.title}*\n     ⭐ ${r.imdbRating || 'N/A'} | 🎞️ ${r.quality || 'N/A'}`;
    }).join('\n\n');
    ...
    if (results[0]?.poster) {
      await socket.sendMessage(sender, {
        ...
        contextInfo: {
          externalAdReply: {
            title: `🎬 CINESUBZ — ${query}`, body: `${searchData.total} ප්‍රතිඵල`,
            mediaType: 1, thumbnail: Buffer.alloc(0),
            sourceUrl: 'https://cinesubz.net', renderLargerThumbnail: false, showAdAttribution: true
          }
        }
      }, { quoted: msg });
    } else {
    ...
    const selectionHandler = async (selUpdate) => {
      try {
        ...
        const selText = (selMsg.message?.conversation || selMsg.message?.extendedTextMessage?.text || '').trim();
        ...
        await socket.sendMessage(sender, { text: `📡 *${chosen.title}* ගේ info ලබා ගනිමින්...` }, { quoted: selMsg });

        // ── TV SHOW PATH ──────────────────────────────────────────
        if (isTV(chosen.link)) {
          const [tvData, epiData] = await Promise.all([
            cineGet('/tvshow',  { url: chosen.link }),
            cineGet('/episode', { url: chosen.link })
          ]);
          ...
          const tvCaption = `📺 *${tv.title}*\n╭━━━━━━━━━━━━━━━━━━━●\n┃ ⭐ *IMDB:* ${tv.imdb_rating}/10\n┃ 📅 *Year:* ${tv.year}\n┃ 🎭 *Genres:* ${(tv.genres || []).slice(0, 3).join(', ')}\n╰━━━━━━━━━━━━━━━━━━━●\n\n${tv.description ? `_${tv.description.slice(0, 180)}..._\n\n` : ''}*📋 Episodes (${episodes.length}):*\n━━━━━━━━━━━━━━━━━━━━\n${epiLines || '_Episode list unavailable_'}\n━━━━━━━━━━━━━━━━━━━━\n*│ Reply with episode number*\n*│00 . ʙᴀᴄᴋ ᴛᴏ ᴍᴇɴᴜ*\n*│94 . ᴄʜᴇᴄᴋ ʙᴏᴛ ꜱᴘᴇᴇᴅ*\n> 🏷️ *KEZU TECH* | _TEAM DCT OFC_`;

          if (tv.poster) {
            await socket.sendMessage(sender, {
              image: { url: tv.poster }, caption: tvCaption,
              contextInfo: {
                externalAdReply: {
                  title: tv.title, body: `⭐ ${tv.imdb_rating} | TV Series`,
                  mediaType: 1, thumbnail: Buffer.alloc(0),
                  sourceUrl: chosen.link, renderLargerThumbnail: false, showAdAttribution: true
                }
              }
            }, { quoted: selMsg });
          } else {
          ...
          // STEP 3a — Episode selection
          const epiSelHandler = async (esUpdate) => {
            try {
              const esMsg  = esUpdate.messages?.[0];
              if (!esMsg?.message || esMsg.key.remoteJid !== sender) return;
              const esText = (esMsg.message?.conversation || esMsg.message?.extendedTextMessage?.text || '').trim();
              ...
              await socket.sendMessage(sender, {
                text: `📡 *Ep ${epiChosen.episode}: ${epiChosen.title}* ගේ downloads ලබා ගනිමින්...`
              }, { quoted: esMsg });
              ...
              if (!epiDetail?.status || !epiDetail?.data?.downloads?.length) {
                await socket.sendMessage(sender, {
                  text: `❌ Episode ${epiChosen.episode} downloads හමු නොවිණි.\n🔗 ${epiChosen.url}`
                }, { quoted: esMsg });
                attachSmH();
                return;
              }

              const result = await showQualityPicker(epiDetail.data.downloads, esMsg, epiChosen.title);
              if (!result) return;
              const { chosen: qChosen, qMsg } = result;
              await sendResolvedLink(qChosen.url, qChosen.quality, qMsg, epiChosen.title);
        ...
        // ── MOVIE PATH ────────────────────────────────────────────
          ...
          const result = await showQualityPicker(dlLinks, selMsg, mv.title);
          if (!result) return;
          const { chosen: qChosen, qMsg } = result;
          await sendResolvedLink(qChosen.url, qChosen.quality, qMsg, mv.title);
        }

      } catch (e) {
        console.error('[CINE selection]', e.message);
        socket.ev.off('messages.upsert', selectionHandler);
        await socket.sendMessage(sender, { text: `❌ Error: ${e.message}` }, { quoted: selMsg }).catch(() => {});
      }
    };
